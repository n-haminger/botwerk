...
BOTWERK_INTERAGENT_HOST are automatically set by the Botwerk framework.
...
def main() -> None:
    ...
    sender = os.environ.get("BOTWERK_AGENT_NAME", "unknown")
    ...
    chat_id = os.environ.get("BOTWERK_CHAT_ID", "")
    topic_id = os.environ.get("BOTWERK_TOPIC_ID", "")
    if chat_id:
    ...
    try:
        ...
        print(
            "Make sure the Botwerk supervisor is running with multi-agent support.", file=sys.stderr
        )
        sys.exit(1)
