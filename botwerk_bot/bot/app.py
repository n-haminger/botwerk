...
from botwerk_bot.bot.callbacks import (
    edit_selector_response,
    mark_button_choice,
    parse_ns_callback,
)
from botwerk_bot.bot.chat_tracker import ChatRecord, ChatTracker
from botwerk_bot.bot.file_browser import (
    file_browser_start,
    handle_file_browser_callback,
    is_file_browser_callback,
)
from botwerk_bot.bot.formatting import markdown_to_telegram_html
from botwerk_bot.bot.handlers import (
    handle_abort,
    handle_abort_all,
    handle_command,
    handle_interrupt,
...
from botwerk_bot.bot.media import (
    has_media,
    is_media_addressed,
    is_message_addressed,
    resolve_media_text,
)
from botwerk_bot.bot.message_dispatch import (
    NonStreamingDispatch,
    StreamingDispatch,
    run_non_streaming_message,
    run_streaming_message,
)
from botwerk_bot.bot.middleware import MQ_PREFIX, AuthMiddleware, SequentialMiddleware
from botwerk_bot.bot.sender import SendRichOpts, send_rich
from botwerk_bot.bot.sender import send_files_from_text as _send_files_from_text
from botwerk_bot.bot.topic import TopicNameCache, get_session_key, get_thread_id
from botwerk_bot.bot.typing import TypingContext as _TypingContext
from botwerk_bot.bot.welcome import (
...
_WELCOME_IMAGE = Path(__file__).resolve().parent / "botwerk_images" / "welcome.png"
_CAPTION_LIMIT = 1024
...
class TelegramBot:
    ...
    def __init__(self, config: AgentConfig, *, agent_name: str = "main") -> None:
        ...
        from botwerk_bot.bus.telegram_transport import TelegramTransport
    ...
    async def _on_startup(self) -> None:
        from botwerk_bot.bot.startup import run_startup
    ...
    async def _send_join_notification(self, chat_id: int) -> None:
        ...
        if not text:
            return
        from botwerk_bot.bot.sender import _send_text_chunks
    ...
    async def _on_agent_commands(self, message: Message) -> None:
        ...
        lines = [
            ...
            "`agents.json` in your botwerk home directory.",
        ]
            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="GitHub", url="https://github.com/n-haminger/botwerk"
                    ),
                    InlineKeyboardButton(
                        text="Changelog",
    ...
    async def _on_showfiles(self, message: Message) -> None:
        """Handle /showfiles: interactive file browser for ~/.botwerk."""
    ...
    async def _on_forum_topic_created(self, message: Message) -> None:
        ...
        from botwerk_bot.bot.topic import get_topic_name_from_message
    ...
    async def _on_session(self, message: Message) -> None:
        ...
        try:
            if session_followup:
                ...
                from botwerk_bot.orchestrator.core import NamedSessionRequest
    ...
    async def _on_restart(self, message: Message) -> None:
        from botwerk_bot.infra.restart import write_restart_sentinel

        chat_id = message.chat.id
        paths = self._orch.paths
        sentinel = paths.botwerk_home / "restart-sentinel.json"
    ...
    async def _route_special_callback(
        ...
        from botwerk_bot.orchestrator.selectors.model_selector import is_model_selector_callback

        if is_model_selector_callback(data):
            await self._handle_model_selector(key, message_id, data)
            return True

        from botwerk_bot.orchestrator.selectors.cron_selector import is_cron_selector_callback
    ...
    async def _route_prefix_callback(
        ...
        from botwerk_bot.orchestrator.selectors.session_selector import is_session_selector_callback
        from botwerk_bot.orchestrator.selectors.task_selector import is_task_selector_callback
    ...
    async def _handle_model_selector(self, key: SessionKey, message_id: int, data: str) -> None:
        ...
        from botwerk_bot.orchestrator.selectors.model_selector import handle_model_callback
    ...
    async def _handle_cron_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from botwerk_bot.orchestrator.selectors.cron_selector import handle_cron_callback
    ...
    async def _handle_session_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from botwerk_bot.orchestrator.selectors.session_selector import handle_session_callback
    ...
    async def _handle_task_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from botwerk_bot.orchestrator.selectors.task_selector import handle_task_callback
    ...
    async def _handle_ns_callback(
        ...
        async with self._sequential.get_lock(key.lock_key):
            if self._config.streaming.enabled:
                from botwerk_bot.orchestrator.flows import named_session_streaming

                result = await named_session_streaming(self._orch, key, session_name, label)
            else:
                from botwerk_bot.orchestrator.flows import named_session_flow
    ...
    async def on_async_interagent_result(self, result: AsyncInterAgentResult) -> None:
        ...
        from botwerk_bot.bus.adapters import from_interagent_result
    ...
    async def on_task_result(self, result: TaskResult) -> None:
        ...
        from botwerk_bot.bus.adapters import from_task_result
    ...
    async def on_task_question(
        ...
        from botwerk_bot.bus.adapters import from_task_question
    ...
    async def _handle_webhook_wake(self, chat_id: int, prompt: str) -> str | None:
        ...
        from botwerk_bot.bus.envelope import LockMode
        ...
        from botwerk_bot.bus.adapters import from_webhook_wake
    ...
    async def _on_update_available(self, info: VersionInfo) -> None:
        ...
        from botwerk_bot.bot.upgrade_handler import on_update_available
    ...
    async def _handle_upgrade_callback(
        ...
        from botwerk_bot.bot.upgrade_handler import handle_upgrade_callback
    ...
    async def _handle_changelog_callback(
        ...
        from botwerk_bot.bot.upgrade_handler import handle_changelog_callback
    ...
    async def _watch_restart_marker(self) -> None:
        ...
        paths = self._orch.paths
        marker = paths.botwerk_home / "restart-requested"
        try:
            while True:
