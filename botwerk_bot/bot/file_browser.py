"""Interactive file browser for the ~/.botwerk directory.

Renders the botwerk home directory as a navigable inline-keyboard tree.
...
async def file_browser_start(paths: BotwerkPaths) -> tuple[str, InlineKeyboardMarkup]:
...
async def handle_file_browser_callback(
    ...
    if data.startswith(SF_FILE_PREFIX):
        ...
        abs_dir = (paths.botwerk_home / rel).resolve() if rel else paths.botwerk_home.resolve()
        prompt = (
...
def _build_view(paths: BotwerkPaths, rel: str) -> tuple[str, InlineKeyboardMarkup]:
    """Build the text + keyboard for a directory listing."""
    base = paths.botwerk_home.resolve()
    ...
    display_path = f"~/.botwerk/{rel}" if rel else "~/.botwerk/"
