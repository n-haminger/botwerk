...
from botwerk_bot.infra.restart import consume_restart_sentinel
from botwerk_bot.infra.updater import UpdateObserver, consume_upgrade_sentinel
...
async def _handle_restart_sentinel(bot: TelegramBot) -> dict[str, object] | None:
    ...
    sentinel_path = bot._orch.paths.botwerk_home / "restart-sentinel.json"
...
async def run_startup(bot: TelegramBot) -> None:
    ...
    from botwerk_bot.bot.chat_tracker import ChatTracker
    ...
    upgrade = await asyncio.to_thread(consume_upgrade_sentinel, bot._orch.paths.botwerk_home)
    if upgrade:
    ...
    from botwerk_bot.infra.startup_state import detect_startup_kind, save_startup_state
    from botwerk_bot.text.response_format import startup_notification_text
    ...
    from botwerk_bot.infra.recovery import RecoveryPlanner
    from botwerk_bot.text.response_format import recovery_notification_text
    ...
    from botwerk_bot.infra.install import is_upgradeable
