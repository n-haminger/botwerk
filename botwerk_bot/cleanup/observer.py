...
class CleanupObserver(BaseObserver):
    """Runs daily file cleanup for media_files, output_to_user, and api_files.
    ...
    async def _execute(self) -> None:
        ...
        targets = [
            (self._paths.media_files_dir, self._cfg.media_files_days),
        ...
        if any(results):
            logger.info(
                "Cleanup complete: media=%d, output=%d, api=%d",
                results[0],
                results[1],
                results[2],
            )
