...
from botwerk_bot.cli.gemini_utils import find_gemini_cli
...
def _gemini_key_auth_source(gemini_home: Path) -> tuple[Path | None, datetime | None]:
    ...
    botwerk_key, botwerk_config_path = read_botwerk_gemini_api_key()
    if botwerk_key and botwerk_config_path is not None:
        return botwerk_config_path, datetime.fromtimestamp(
            botwerk_config_path.stat().st_mtime, tz=UTC
        )
    return None, None
...
def read_botwerk_gemini_api_key() -> tuple[str | None, Path | None]:
    """Read ``gemini_api_key`` from ``~/.botwerk/config/config.json``.

    Returns ``(key, path)`` when configured, otherwise ``(None, None)``.
    """
    config_path = _botwerk_config_path()
