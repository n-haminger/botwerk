...
from botwerk_bot.cli.base import (
    _IS_WINDOWS,
    CLIConfig,
    _win_feed_stdin,
)
from botwerk_bot.cli.stream_events import ResultEvent, StreamEvent
...
def _build_subprocess_env(config: CLIConfig) -> dict[str, str] | None:
    ...
    if config.topic_id:
        env["BOTWERK_TOPIC_ID"] = str(config.topic_id)
    working_dir = Path(config.working_dir)
    botwerk_home = working_dir.parent if working_dir.name == "workspace" else working_dir
    env["BOTWERK_HOME"] = str(botwerk_home)
    # Shared knowledge is always at the main agent's home level.
    # For main: botwerk_home itself. For sub-agents: ../../ from agents/<name>/.
    if config.agent_name == "main":
        env["BOTWERK_SHARED_MEMORY_PATH"] = str(botwerk_home / "SHAREDMEMORY.md")
    else:
        # Sub-agent home is <main_home>/agents/<name>/
        main_home = botwerk_home.parent.parent
        env["BOTWERK_SHARED_MEMORY_PATH"] = str(main_home / "SHAREDMEMORY.md")
    return env
