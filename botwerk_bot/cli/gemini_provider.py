...
from botwerk_bot.cli.auth import gemini_api_key_mode_selected
...
from botwerk_bot.cli.stream_events import ResultEvent, StreamEvent, SystemInitEvent
...
class GeminiCLI(BaseCLI):
    ...
    def _inject_config_gemini_api_key(self, env: dict[str, str]) -> None:
        """Inject GEMINI_API_KEY from botwerk config when API-key auth mode is active."""
        ...
        logger.debug("Injected GEMINI_API_KEY from botwerk config for Gemini API key mode")
