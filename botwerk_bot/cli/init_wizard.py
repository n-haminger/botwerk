...
def _show_disclaimer(console: Console) -> None:
    ...
    disclaimer = (
        ...
        "[bold green]We strongly recommend running botwerk inside a Docker container[/bold green] "
    ...
    if docker_found:
        console.print(
            Panel(
                ...
                "Running botwerk inside Docker isolates it from your host.\n"
...
def _build_extras_table(console: Console) -> None:
    ...
    from botwerk_bot.infra.docker_extras import DOCKER_EXTRAS_BY_ID, extras_for_display
    ...
    console.print(
        ...
        " [cyan]botwerk docker extras-add / extras-remove[/cyan].[/dim]"
    )
    console.print()
...
def run_smart_reset(botwerk_home: Path) -> None:
    ...
    config_path = botwerk_home / "config" / "config.json"
    ...
    if config_path.exists():
        try:
            ...
            if isinstance(docker, dict) and docker.get("enabled"):
                docker_container = str(docker.get("container_name", "botwerk-sandbox"))
                docker_image = str(docker.get("image_name", "botwerk-sandbox"))
        except (json.JSONDecodeError, OSError):
            pass
