...
from botwerk_bot.cli.factory import create_cli
...
from botwerk_bot.cli.types import AgentRequest, AgentResponse, CLIResponse

if TYPE_CHECKING:
    from botwerk_bot.cli.base import BaseCLI
    ...
    async def dispatch(self, event: StreamEvent) -> tuple[str, ResultEvent | None]:
        ...
        if isinstance(event, ThinkingEvent) and self._on_status is not None:
            ...
            if self._on_compaction is not None:
                self._on_compaction(event.pre_tokens)
...
class CLIService:
    ...
    async def execute_streaming(
        ...
        callbacks = _StreamCallbacks(
            on_text_delta, on_tool_activity, on_system_status, on_compaction
        )
