"""Agent management CLI subcommands (``botwerk agents ...``)."""
...
def _parse_agents_subcommand(args: list[str]) -> tuple[str | None, list[str]]:
    ...
    for a in args:
        ...
        if found and sub is not None:
            rest.append(a)
    if found and sub is None:
        # bare "botwerk agents" → default to list
        return "list", []
    return sub, rest
...
def print_agents_help() -> None:
    ...
    table.add_column()
    table.add_row("botwerk agents", "List all sub-agents and their config")
    table.add_row("botwerk agents list", "List all sub-agents and their config")
    table.add_row("botwerk agents add <name>", "Add a new sub-agent (interactive)")
    table.add_row("botwerk agents remove <name>", "Remove a sub-agent")
    _console.print(
...
def load_agents_registry(paths: BotwerkPaths) -> list[dict[str, object]]:
    """Load sub-agent definitions from agents.json (raw dicts)."""
    agents_path = paths.botwerk_home / "agents.json"
...
def fetch_live_health() -> dict[str, dict[str, object]]:
    ...
    from botwerk_bot.multiagent.internal_api import _DEFAULT_PORT
...
def validate_agent_name(name: str | None, agents: list[dict[str, object]]) -> str | None:
    """Validate an agent name for ``botwerk agents add``. Returns clean name or None on error."""
    if not name:
        _console.print("[bold red]Usage: botwerk agents add <name>[/bold red]")
        return None
...
def agents_list() -> None:
    ...
    if not agents:
        ...
        _console.print("[dim]Use botwerk agents add <name>' to create one.[/dim]")
        return
    ...
    if pid_file.exists():
        try:
            ...
            from botwerk_bot.infra.pidlock import _is_process_alive
...
def agents_remove(rest: list[str]) -> None:
    ...
    if not name:
        _console.print("[bold red]Usage: botwerk agents remove <name>[/bold red]")
        return
    ...
    _console.print(f"[dim]Workspace data remains at {paths.botwerk_home / 'agents' / name}[/dim]")


def cmd_agents(args: list[str]) -> None:
    """Handle botwerk agents [subcommand]'."""
