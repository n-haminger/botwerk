"""API server management CLI subcommands (``botwerk api ...``)."""
...
from botwerk_bot.config import _BIND_ALL_INTERFACES
...
def print_api_help() -> None:
    ...
    table.add_column()
    table.add_row("botwerk api enable", "Enable the WebSocket API server")
    table.add_row("botwerk api disable", "Disable the WebSocket API server")
...
def api_install_hint() -> str:
    ...
    if mode == "pipx":
        return "pipx inject botwerk PyNaCl"
    ...
    if not nacl_available():
        ...
        _console.print(
            Panel(
                ...
                f"\n\nInstall it with:\n\n  [bold]{hint}[/bold]"
                "\n\nThen run [bold]botwerk api enable[/bold] again.",
