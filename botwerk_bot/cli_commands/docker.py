"""Docker management CLI subcommands (``botwerk docker ...``)."""
...
def _parse_docker_mount_arg(args: list[str]) -> str | None:
    ...
    Expects the form: ``botwerk docker mount <path>`` where *args* is
    ``sys.argv[1:]`` (no ``botwerk`` prefix).  Non-flag positionals are
...
def print_docker_help() -> None:
    ...
    table.add_column()
    table.add_row("botwerk docker rebuild", "Remove container & image, rebuild on next start")
    table.add_row("botwerk docker enable", "Enable Docker sandboxing")
    table.add_row("botwerk docker disable", "Disable Docker sandboxing")
    table.add_row("botwerk docker mount <path>", "Mount a host directory into the sandbox")
    table.add_row("botwerk docker unmount <path>", "Remove a mounted directory")
    table.add_row("botwerk docker mounts", "List all mounted directories")
    table.add_row("botwerk docker extras", "List available and installed extras")
    table.add_row("botwerk docker extras-add <id>", "Add an extra package")
    table.add_row("botwerk docker extras-remove <id>", "Remove an extra package")
    _console.print(
...
def docker_read_config() -> tuple[Path, dict[str, object]] | None:
    ...
    if not config_path.exists():
        _console.print("[bold red]Config not found. Run botwerk first.[/bold red]")
        return None
    try:
...
def docker_rebuild() -> None:
    ...
    result = docker_read_config()
    container = "botwerk-sandbox"
    image = "botwerk-sandbox"
...
def docker_mount(args: list[str]) -> None:
    ...
    if not raw_path:
        _console.print("[bold red]Usage: botwerk docker mount <path>[/bold red]")
        return
    ...
    from botwerk_bot.infra.docker import resolve_mount_target
...
def docker_unmount(args: list[str]) -> None:
    ...
    raw_path = _parse_docker_mount_arg(args)
    if not raw_path:
        _console.print("[bold red]Usage: botwerk docker unmount <path>[/bold red]")
        return
...
def docker_list_mounts() -> None:
    ...
    if not isinstance(mounts, list) or not mounts:
        _console.print("[dim]No mounts configured.[/dim]")
        _console.print("[dim]Use botwerk docker mount <path>' to add one.[/dim]")
        return
...
def docker_container_name() -> str:
    ...
    result = docker_read_config()
    if result is None:
        return "botwerk-sandbox"
    _, data = result
    docker = data.get("docker", {})
    if isinstance(docker, dict):
        return str(docker.get("container_name", "botwerk-sandbox"))
    return "botwerk-sandbox"
...
def docker_extras_list() -> None:
    ...
    from botwerk_bot.infra.docker_extras import extras_for_display
    ...
    if selected:
        _console.print()
        _console.print(
            "[dim]Run botwerk docker rebuild' to apply selected extras to the image.[/dim]"
        )
    _console.print()
    _console.print("[dim]Use botwerk docker extras-add <id>' to add an extra.[/dim]")
    _console.print("[dim]Use botwerk docker extras-remove <id>' to remove an extra.[/dim]")


def docker_extras_add(args: list[str]) -> None:
    """Add an extra to the Docker config."""
    from botwerk_bot.infra.docker_extras import DOCKER_EXTRAS_BY_ID

    positionals = [a for a in args if not a.startswith("-")]
    extra_id = positionals[2] if len(positionals) >= 3 else None

    if not extra_id:
        _console.print("[bold red]Usage: botwerk docker extras-add <id>[/bold red]")
        _console.print("[dim]Run botwerk docker extras' to see available IDs.[/dim]")
        return
    ...
    _console.print("[yellow]Run botwerk docker rebuild' to apply.[/yellow]")


def docker_extras_remove(args: list[str]) -> None:
    """Remove an extra from the Docker config."""
    from botwerk_bot.infra.docker_extras import DOCKER_EXTRAS, DOCKER_EXTRAS_BY_ID

    positionals = [a for a in args if not a.startswith("-")]
    extra_id = positionals[2] if len(positionals) >= 3 else None

    if not extra_id:
        _console.print("[bold red]Usage: botwerk docker extras-remove <id>[/bold red]")
        _console.print("[dim]Run botwerk docker extras' to see installed IDs.[/dim]")
        return
...
def cmd_docker(args: list[str]) -> None:
    """Handle botwerk docker <subcommand>'."""
