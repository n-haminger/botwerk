...
_BOTWERK_MOUNT = "/botwerk"
_CONTAINER_WS = f"{_BOTWERK_MOUNT}/workspace"
_MOUNT_PREFIX = "/mnt"
...
class DockerManager:
    ...
    def __init__(self, config: DockerConfig, paths: BotwerkPaths) -> None:
        self._config = config
        self._paths = paths
    ...
    async def _build_image(self, image: str) -> bool:
        ...
        from botwerk_bot.infra.docker_extras import (
    ...
    async def _start_container(self, name: str, image: str) -> bool:
        # Always mount the root botwerk home, even when called from a sub-agent.
        ...
        # like /botwerk/agents/<name>/workspace.
        botwerk_home = self._paths.botwerk_home
        if botwerk_home.parent.name == "agents":
            botwerk_home = botwerk_home.parent.parent
        ...
        cmd: list[str] = [
            ...
            "-w",
            _CONTAINER_WS,
            # Mount the ENTIRE ~/.botwerk so the CLI sees all framework files.
            "-v",
            f"{botwerk_home}:{_BOTWERK_MOUNT}",
            "-e",
            f"BOTWERK_HOME={_BOTWERK_MOUNT}",
    ...
    def _env_secret_flags(self) -> list[str]:
        """Return ``-e`` flags for user secrets from ``~/.botwerk/.env``."""
