...
def find_botwerk_binary() -> str | None:
    """Find the botwerk binary in PATH. Shared across all backends."""
    return shutil.which("botwerk")
...
_NOT_INSTALLED_MSG = "[dim]Service not installed. Run [bold]botwerk service install[/bold].[/dim]"
_NOT_RUNNING_MSG = "[dim]Service is not running.[/dim]"
_NO_SERVICE_MSG = "[dim]No service installed.[/dim]"
_BINARY_NOT_FOUND_MSG = "[bold red]Could not find the botwerk binary in PATH.[/bold red]"
...
def print_binary_not_found(console: Console) -> None:
    """Print the botwerk binary not found' error."""
...
def print_install_success(
    ...
    *logs_hint* is the description next to ``botwerk service logs``.
    """
    console.print(
        Panel(
            "[bold green]botwerk is now running as a background service.[/bold green]\n\n"
            f"{detail}\n\n"
            "[bold]Useful commands:[/bold]\n\n"
            "  [cyan]botwerk service status[/cyan]     Check if it's running\n"
            "  [cyan]botwerk service stop[/cyan]       Stop the service\n"
            f"  [cyan]botwerk service logs[/cyan]       {logs_hint}\n"
            "  [cyan]botwerk service uninstall[/cyan]  Remove the service",
