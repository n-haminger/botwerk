"""Systemd user service management for botwerk (Linux)."""
...
from botwerk_bot.infra.service_logs import print_journal_service_logs
...
Environment=BOTWERK_SUPERVISOR=1
...
def is_service_installed() -> bool:
    """Check if the botwerk service is installed."""
    return _service_path().exists()


def is_service_running() -> bool:
    """Check if the botwerk service is currently running."""
...
def install_service(console: Console | None = None) -> bool:
    """Install and start the botwerk systemd user service.
    ...
    if not _has_linger():
        console.print(
            "\n[bold yellow]Linger must be enabled so botwerk keeps running "
...
def uninstall_service(console: Console | None = None) -> bool:
    """Stop, disable, and remove the botwerk service."""
    console = ensure_console(console)
