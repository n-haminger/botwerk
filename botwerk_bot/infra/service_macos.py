"""macOS launchd Launch Agent service management for botwerk."""
...
def is_service_installed() -> bool:
    """Check if the botwerk Launch Agent plist exists."""
    return _plist_path().exists()


def is_service_running() -> bool:
    """Check if the botwerk Launch Agent is currently running."""
...
def install_service(console: Console | None = None) -> bool:
    """Install and start the botwerk Launch Agent.
...
def uninstall_service(console: Console | None = None) -> bool:
    """Stop and remove the botwerk Launch Agent."""
    console = ensure_console(console)
...
def print_service_status(console: Console | None = None) -> None:
    ...
    if result.returncode == 0:
        ...
        console.print("[red]Agent not loaded. Try: [bold]botwerk service install[/bold][/red]")
