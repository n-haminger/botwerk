"""Windows Task Scheduler service management for botwerk."""
...
from botwerk_bot.infra.service_base import (
    ensure_console,
    find_botwerk_binary,
...
from botwerk_bot.infra.service_logs import print_file_service_logs
...
_ADMIN_HINT_PANEL = Panel(
    ...
    "  [bold]3.[/bold] Run: [cyan]botwerk service install[/cyan]",
...
def _task_xml_path() -> Path:
    ...
    return resolve_paths().botwerk_home / "botwerk_task.xml"
...
def is_service_installed() -> bool:
    """Check if the botwerk scheduled task exists."""
    result = _run_schtasks("/Query", "/TN", _TASK_NAME, "/FO", "LIST")
    return result.returncode == 0


def is_service_running() -> bool:
    """Check if the botwerk scheduled task is currently running."""
...
def install_service(console: Console | None = None) -> bool:
    """Install and start the botwerk scheduled task.
    ...
    # Resolve command: prefer pythonw.exe (no console window) over botwerk binary
    pythonw = _find_pythonw()
    if pythonw:
        command = pythonw
        arguments = "-m botwerk_bot"
    else:
        binary = find_botwerk_binary()
        if not binary:
...
def uninstall_service(console: Console | None = None) -> bool:
    """Stop and remove the botwerk scheduled task."""
    console = ensure_console(console)
...
def print_service_logs(console: Console | None = None) -> None:
    ...
    Windows has no journalctl equivalent, so we tail the botwerk log file.
    """
    console = ensure_console(console)
