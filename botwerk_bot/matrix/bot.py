...
from botwerk_bot.matrix.buttons import ButtonTracker
from botwerk_bot.matrix.credentials import login_or_restore
from botwerk_bot.matrix.id_map import MatrixIdMap
from botwerk_bot.matrix.sender import send_rich as matrix_send_rich
...
class MatrixBot:
    ...
    def __init__(self, config: AgentConfig, *, agent_name: str = "main") -> None:
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install 'botwerk[matrix]'"
            ) from None
        ...
        from botwerk_bot.matrix.transport import MatrixTransport
    ...
    async def run(self) -> int:
        ...
        from botwerk_bot.matrix.startup import run_matrix_startup
    ...
    async def _on_media(self, room: MatrixRoom | object, event: RoomMessageMedia | object) -> None:
        ...
        from botwerk_bot.matrix.media import resolve_matrix_media
    ...
    async def _send_selector_response(
        ...
        from botwerk_bot.matrix.buttons import REACTION_DIGITS
    ...
    async def _handle_button_callback(
        ...
        from botwerk_bot.orchestrator.selectors.session_selector import (
            handle_session_callback,
            is_session_selector_callback,
        )
        from botwerk_bot.orchestrator.selectors.task_selector import (
    ...
    async def _watch_restart_marker(self) -> None:
        ...
        marker = Path(self._config.botwerk_home).expanduser() / "restart-requested"
        while True:
