...
if TYPE_CHECKING:
    ...
    from botwerk_bot.multiagent.shared_knowledge import SharedKnowledgeSync
...
class AgentSupervisor:
    ...
    def __init__(self, main_config: AgentConfig) -> None:
        ...
        self._main_paths = resolve_paths(botwerk_home=main_config.botwerk_home)
        self._agents_path = self._main_paths.botwerk_home / "agents.json"
    ...
    async def start(self) -> int:
        ...
        shared_path = self._main_paths.botwerk_home / "SHAREDMEMORY.md"
    ...
    async def _start_sub_agent(self, sub_cfg: SubAgentConfig) -> None:
        ...
        agent_home = self._main_paths.botwerk_home / "agents" / name
    ...
    _PROVISIONING_SCRIPT = "/opt/botwerk/scripts/manage-agent-user.sh"
    ...
    async def _ensure_agent_user(self, agent_name: str) -> None:
        ...
        if proc.returncode != 0:
            logger.error(
                "Failed to provision user botwerk-%s: %s",
                agent_name,
                stderr.decode(errors="replace").strip(),
            )
        else:
            logger.info(
                "Agent user botwerk-%s: %s",
                agent_name,
                stdout.decode(errors="replace").strip(),
            )

    async def _fix_agent_perms(self, agent_name: str) -> None:
        """Ensure the botwerk user owns the workspace and the agent user has ACL access."""
        import getpass
