...
from botwerk_bot.orchestrator.selectors.models import Button, ButtonGrid
from botwerk_bot.orchestrator.selectors.session_selector import session_selector_start
from botwerk_bot.orchestrator.selectors.task_selector import task_selector_start
from botwerk_bot.text.response_format import SEP, fmt, new_session_text
...
def _resolve_log_path(orch: Orchestrator) -> Path:
    ...
    log in the main botwerk home (parent of ``agents/<name>``).
    """
    log_path = orch.paths.logs_dir / "agent.log"
    if not log_path.exists():
        main_logs = orch.paths.botwerk_home.parent.parent / "logs" / "agent.log"
        if main_logs.exists():
            return main_logs
    return log_path
