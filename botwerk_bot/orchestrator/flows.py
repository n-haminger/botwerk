...
from botwerk_bot.cli.timeout_controller import TimeoutConfig as TCConfig
...
from botwerk_bot.config import NULLISH_TEXT_VALUES, resolve_timeout
from botwerk_bot.infra.inflight import InflightTurn
from botwerk_bot.log_context import set_log_context
from botwerk_bot.orchestrator.hooks import HookContext
from botwerk_bot.orchestrator.registry import OrchestratorResult
from botwerk_bot.session import SessionData, SessionKey
from botwerk_bot.text.response_format import session_error_text, timeout_error_text
from botwerk_bot.workspace.loader import read_mainmemory
...
def _is_invalid_session(response: AgentResponse) -> bool:
    ...
    Happens when sessions reference a stale working directory.
    """
...
async def _gemini_missing_config_key_warning(
    ...
    """Warn when Gemini API-key mode is selected but Botwerk config key is empty."""
    ...
    return OrchestratorResult(
        text=(
            ...
            '`~/.botwerk/config/config.json` is `"null"` or empty.\n'
            "Why this is required: when botwerk calls Gemini CLI as an external process, "
            "Gemini CLI does not expose an internally entered API key to that caller.\n"
            "Set a real API key in `gemini_api_key` and restart `botwerk`."
        ),
    )
...
async def normal(
    ...
    *,
    ...
    if warning is not None:
        logger.warning("Gemini API-key mode without configured botwerk key")
        return warning
    ...
    try:
        ...
        if _reg.was_aborted(key.chat_id):
