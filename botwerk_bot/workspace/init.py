...
from botwerk_bot.infra.atomic_io import atomic_text_save
from botwerk_bot.workspace.cron_tasks import ensure_task_rule_files
from botwerk_bot.workspace.paths import BotwerkPaths
from botwerk_bot.workspace.rules_selector import RulesSelector
from botwerk_bot.workspace.skill_sync import sync_bundled_skills, sync_skills
...
def _sync_home_defaults(paths: BotwerkPaths) -> None:
    """Walk the home-defaults template and copy to ``botwerk_home``.

    The template at ``<repo>/workspace/`` mirrors ``~/.botwerk/`` exactly.
    Zone rules per file:
    ...
    if not paths.home_defaults.is_dir():
        ...
        return
    _walk_and_copy(paths.home_defaults, paths.botwerk_home)
    ...
    if paths.botwerk_home.parent.name != "agents":
...
def _smart_merge_config(paths: BotwerkPaths) -> None:
...
def _migrate_tasks_to_cron_tasks(paths: BotwerkPaths) -> None:
...
def _clean_orphan_symlinks(paths: BotwerkPaths) -> None:
...
def _ensure_required_dirs(paths: BotwerkPaths) -> None:
    """Create any required directories that are missing."""
    for rel in _REQUIRED_DIRS:
        d = paths.botwerk_home / rel
        if not d.is_dir():
            d.mkdir(parents=True, exist_ok=True)
            logger.info("Created missing directory: %s", d)


def init_workspace(paths: BotwerkPaths) -> None:
    """Initialize the workspace: defaults sync, rule sync, config merge, cleanup."""
    logger.info("Workspace init started home=%s", paths.botwerk_home)
...
- Replies are displayed in the Web UI (no hard character limit, but keep responses readable).
- Messages are formatted as Markdown (rendered by the Web UI).
...
Keep labels short and distinctive.
...
- Each sub-agent has its own **bot** with a separate chat.
...
- You have your own workspace, memory, and chat interface.
...
def inject_runtime_environment(
    ...
    *,
    ...
    Called once after workspace init when the transport is known.
    """
    identity_notice = _build_identity_notice(agent_name)
    transport_notice = _TRANSPORT_WEBUI
    ...
    for name in _RULE_FILE_NAMES:
        ...
        if "## Multi-Agent Identity" in content or "## Runtime Environment" in content:
            continue
        atomic_text_save(target, content + transport_notice + identity_notice + _HOST_NOTICE)
    logger.info(
        "Runtime environment injected: agent=%s transport=%s",
        agent_name,
        transport,
    )
