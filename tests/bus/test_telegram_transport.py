...
from botwerk_bot.bus.cron_sanitize import (
    is_cron_transport_ack_line,
    sanitize_cron_result_text,
)
from botwerk_bot.bus.envelope import Envelope, Origin
...
class TestCronBroadcast:
    async def test_broadcasts_with_result_text(self) -> None:
        ...
        with patch("botwerk_bot.bus.telegram_transport.send_rich", new_callable=AsyncMock):
...
class TestHeartbeatDelivery:
    async def test_delivers_to_chat(self) -> None:
        ...
        with patch(
            "botwerk_bot.bus.telegram_transport.send_rich", new_callable=AsyncMock
        ) as mock_send:
