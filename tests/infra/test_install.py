...
from botwerk_bot.infra.install import detect_install_mode, is_upgradeable
...
class TestDetectInstallMode:
    ...
    def test_pipx_detected_from_sys_prefix(self) -> None:
        with patch("botwerk_bot.infra.install.sys") as mock_sys:
            mock_sys.prefix = "/home/user/.local/share/pipx/venvs/botwerk"
    ...
    def test_editable_install_detected_as_dev(self) -> None:
        ...
        with (
            patch("botwerk_bot.infra.install.sys") as mock_sys,
            patch("botwerk_bot.infra.install.distribution", return_value=mock_dist),
        ):
    ...
    def test_package_not_found_is_dev(self) -> None:
        ...
        with (
            patch("botwerk_bot.infra.install.sys") as mock_sys,
            patch(
                "botwerk_bot.infra.install.distribution",
                side_effect=PackageNotFoundError("botwerk"),
            ),
        ):
            mock_sys.prefix = "/usr"
            assert detect_install_mode() == "dev"
    ...
    def test_metadata_error_falls_back_to_dev(self) -> None:
        with (
            patch("botwerk_bot.infra.install.sys") as mock_sys,
            patch("botwerk_bot.infra.install.distribution", side_effect=OSError("corrupt")),
        ):
            mock_sys.prefix = "/usr"
            assert detect_install_mode() == "dev"
    ...
    def test_pipx_path_variant_windows(self) -> None:
        with patch("botwerk_bot.infra.install.sys") as mock_sys:
            mock_sys.prefix = "C:\\Users\\me\\AppData\\Local\\pipx\\venvs\\botwerk"
            assert detect_install_mode() == "pipx"
...
class TestIsUpgradeable:
    ...
    def test_pipx_is_upgradeable(self) -> None:
        with patch("botwerk_bot.infra.install.detect_install_mode", return_value="pipx"):
            assert is_upgradeable() is True

    def test_pip_is_upgradeable(self) -> None:
        with patch("botwerk_bot.infra.install.detect_install_mode", return_value="pip"):
            assert is_upgradeable() is True

    def test_dev_is_not_upgradeable(self) -> None:
        with patch("botwerk_bot.infra.install.detect_install_mode", return_value="dev"):
