...
from botwerk_bot.infra.service_linux import (
    _SERVICE_NAME,
...
class TestGenerateServiceUnit:
    def test_contains_binary_path(self) -> None:
        unit = _generate_service_unit("/usr/local/bin/botwerk")
        assert "ExecStart=/usr/local/bin/botwerk" in unit

    def test_has_restart_policy(self) -> None:
        unit = _generate_service_unit("botwerk")
    ...
    def test_includes_all_nvm_bins(self, tmp_path: Path) -> None:
        ...
        with patch("botwerk_bot.infra.service_linux.Path.home", return_value=tmp_path):
            unit = _generate_service_unit("botwerk")
...
class TestIsServiceAvailable:
    @patch("botwerk_bot.infra.service_linux.shutil.which", return_value="/usr/bin/systemctl")
    def test_available_with_systemctl(self, _mock: MagicMock) -> None:
        assert is_service_available() is True

    @patch("botwerk_bot.infra.service_linux.shutil.which", return_value=None)
...
class TestIsServiceRunning:
    @patch("botwerk_bot.infra.service_linux._run_systemctl")
    @patch("botwerk_bot.infra.service_linux.is_service_installed", return_value=True)
    @patch("botwerk_bot.infra.service_linux._has_systemd", return_value=True)
...
class TestStartService:
    @patch("botwerk_bot.infra.service_linux._run_systemctl")
    @patch("botwerk_bot.infra.service_linux._has_systemd", return_value=False)
    ...
    @patch("botwerk_bot.infra.service_linux._run_systemctl")
    @patch("botwerk_bot.infra.service_linux.is_service_installed", return_value=False)
    @patch("botwerk_bot.infra.service_linux._has_systemd", return_value=True)
    def test_start_not_installed(
        ...
        console.print.assert_called_with(
            "[dim]Service not installed. Run [bold]botwerk service install[/bold].[/dim]"
        )
...
class TestStopService:
    @patch("botwerk_bot.infra.service_linux._run_systemctl")
    @patch("botwerk_bot.infra.service_linux.is_service_running", return_value=True)
...
class TestUninstallService:
    ...
    @patch("botwerk_bot.infra.service_linux._run_systemctl")
    @patch("botwerk_bot.infra.service_linux._service_path")
    @patch("botwerk_bot.infra.service_linux.is_service_installed", return_value=True)
    @patch("botwerk_bot.infra.service_linux._has_systemd", return_value=True)
    def test_uninstall(
        self,
