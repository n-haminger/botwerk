...
from botwerk_bot.infra.version import VersionInfo
...
class TestPerformUpgradePipeline:
    ...
    async def test_changes_on_first_attempt(self) -> None:
        with (
            patch(
                "botwerk_bot.infra.updater._perform_upgrade_impl",
                new=AsyncMock(return_value=(True, "first-pass")),
            ) as mock_upgrade,
            patch(
                "botwerk_bot.infra.updater._wait_for_version_change",
    ...
    async def test_returns_unchanged_when_no_retry_target(self) -> None:
        with (
            ...
            patch(
                "botwerk_bot.infra.updater._resolve_retry_target",
